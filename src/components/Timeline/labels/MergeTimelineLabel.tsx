...
import { generateMergeMarble } from '../../../utils/marbleUtils';
...
export const MergeTimelineLabel: React.FC<TimelineLabelProps> = ({ label, streamId }) => {
  const { addMarble, isStream3Enabled } = useMarbleStore();

  const handleAddMarble = () => {
    if (!streamId) return;
    const marble = generateMergeMarble(streamId);
