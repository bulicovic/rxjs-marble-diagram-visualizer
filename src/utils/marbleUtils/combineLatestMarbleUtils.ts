import { createMarble, letters, specialChars } from './baseMarbleUtils';
...
export const generateCombineLatestMarble = (streamId: number) => {
  switch (streamId) {
    case 1: {
      ...
      letterIndex++;
      return createMarble(letter, 'rgb(59, 130, 246)');
    } // blue
    case 2: {
      const number = numberIndex++;
      return createMarble(number, 'rgb(239, 68, 68)');
    } // red
    case 3: {
      const special = specialChars[specialCharIndex % specialChars.length];
      specialCharIndex++;
      return createMarble(special, 'rgb(168, 85, 247)');
    } // purple
    default:
