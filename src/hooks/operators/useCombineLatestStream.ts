...
export const useCombineLatestStream = () => {
  const { stream1$, stream2$, stream3$, isStream3Enabled, addOutputMarble } =
  ...
  useEffect(() => {
    ...
    const subscription = combineLatest(streams$)
      .pipe(
        map(combined => ({
          id: Object.values(combined)
            .map(m => m.id)
            .join('-'),
          value: Object.values(combined)
            .map(m => m.value)
            .join(''),
          timestamp: Date.now(),
          color: 'rgb(34, 197, 94)',
